...
def generate_launch_description():
  ...
  map_directory = os.path.join(get_package_share_directory(
        'tortoisebot_bringup'), 'maps','field.yaml')
  ...
  rviz_node = launch_ros.actions.Node(
    ...
    filtered_lidar_node = Node(
    package='laser_filters',
    executable='scan_to_scan_filter_chain',
    name='laser_filter_node',
    parameters=[os.path.join(get_package_share_directory('tortoisebot_navigation'), 'config', 'laser_filter.yaml')],
    remappings=[
        ('/scan', '/filtered_scan')  # Input from raw scan, output to filtered scan
    ])
